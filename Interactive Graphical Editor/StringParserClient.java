 ...
 * Class to help with handling of messages communicated to the client side.
 */

public class StringParserClient {p
    /**
     ...
     * @param editor An object of type editor (common editor) on which all clients are making changes
     */
    public static void parseString(String input, Editor editor) {
        ...
        if (commandType.equals("DRAW")) {
            ...
            if (shapeType.equals("freehand")) {
                ...
                editor.getSketch().addShape(polyline); // Updating the state of the world (adding shape to canvas)
            }
                ...
                if (shapeType.equals("ellipse")) {
                    ...
                    editor.getSketch().addShape(ellipse); // Updating the state of the world
                }
                ...
                // Checking if  shape type is rectangle
                    ...
                    editor.getSketch().addShape(rectangle); // Updating the state of the world
                }
                ...
                // Checking if  shape type is segment
                    ...
                    editor.getSketch().addShape(segment); // Updating the state of the world
                }
            }
        }
        ...
        // Checking if mode is MOVE
            ...
            Shape shape = editor.getSketch().getShapeNavigableMap().get(shapeID); // Getting the shape from the sketch
        ...
        // Checking if mode is RECOLOR
            ...
            editor.getSketch().getShapeNavigableMap().get(shapeID).setColor(newColor); // Updating a shape's color on the editor canvas
        }

        // Checking if mode is DELETE
        else if (commandType.equals("DELETE")) {
            int shapeID = Integer.parseInt(inputContents[1]); // Getting the shape ID from input
            editor.getSketch().removeShape(shapeID); // Removing the shape from the state of the world
        }

        editor.repaint(); // Repainting the canvas after changes
    }
}
